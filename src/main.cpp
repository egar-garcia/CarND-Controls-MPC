...
#define LATENCY_IN_MILLISECONDS 10
#define WAY_POINTS_POLYNOMIAL_ORDER 2
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          double cte = coeffs[0];  // polinomial evaluated when px = 0, ideal would be 0 (i.e py = 0)
          double epsi = -atan(coeffs[1]);  // using derivative of polinomial when px = 0
          ...
          */
          ...
          // Displaying the waypoints/reference line
          ...
          // SUBMITTING.
          this_thread::sleep_for(chrono::milliseconds(LATENCY_IN_MILLISECONDS));
